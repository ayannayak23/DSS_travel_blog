/**
 * To verify the password hashing helper stores salted bcrypt hashes and validates passwords correctly.
 */
...
async function run() {
    ...
    // To hash the same password twice to confirm bcrypt uses a different salt each time.
    const firstHash = await hashPassword(plainPassword);
    const secondHash = await hashPassword(plainPassword);

    // To ensure the stored value is a bcrypt hash, not the original password.
    ...
    // To verify the right password is accepted and the wrong one is rejected.
